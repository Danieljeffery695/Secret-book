...
<script type="text/javascript">
 ...
 function ajaxCallRequest() {
    $(".comment-btn").click(function() {
        ...
        $(".comment-container-div").load("friend_profile_post_comment1.php",
...
function ajaxCallRequest_2(url) {
    ...
    function ajaxCallRequest_3() {
        $(".post-like-i").click(function() {
          ...
          user_element.load("friend_profile_post_like1.php", {user_profile_like: user_id, profile_post_comment_id: user_comment_id, profile_comment_post: user_story_post })  
        });
    };
    ...
    function ajaxCallFunction11() {
      $(".story-comment-btn").click(function() {
        ...
        var user_comment_id = $(this).data("comment-owner-id");
        var user_story_post = $(this).data("user-story-post");
        $(".comment-container-div").load("friend_profile_story_comment1.php", 
    ...
    function ajaxCallFunction12() {
        $(".story-like-i").click(function() {
          ...
          var user_story_post = $(this).data("user-story-post");
          var user_element = $(this).parent().find("sup").addClass("reaction_sup");
          user_element.load("friend_profile_story_like1.php", {user_profile_like: user_id, profile_story_comment_id: user_comment_id, profile_comment_story: user_story_post })  
        });
    };
    ...
    function gettingUsername_1() {
        $(".add_friend_btn_1").click(function() {
            var username = $(this).data("friend-username");
            $.post("", {friend_username_1: username}, function() {
                console.log("SecretBook is the best!!");
            })
        });
    };
...
if(isset($_POST["friend_username"])) {
  ...
  if($add_friend_row >= 1) {
      ...
      $insert_notification_stmt->bind_param("isisssi", $user_unique_id, $username_33, $request_friend_id, $request_friend_username, $post_comment, $post_unseen, $notification_post_id);
      $insert_notification_stmt->execute();
  } 
}

if(isset($_POST["friend_username_1"])) {
  $friend_username = $_POST["friend_username_1"];
  $select_add_friend = "select * from `secret_users` where username = '$friend_username'";
  $result_add_friend = mysqli_query($con, $select_add_friend);
  $add_friend_row = mysqli_num_rows($result_add_friend);
  if($add_friend_row >= 1) {
      ...
      values (?, ?, ?, ?, ?, NOW())");
      $add_request_stmt = $con->prepare($add_request);
      $add_request_stmt->bind_param("issis", $request_friend_id, $request_friend_username, $username_33, $user_unique_id, $request_status);
      $add_request_stmt->execute();
      echo "<script>alert('successfully uploaded')</script>";
...
if(isset($_POST["first_id_1"])) {
    $first_id = $_POST["first_id_1"];
    $second_id = $_POST["second_id_1"];
    $post_comment = "friend request";
    $delete_friend_request = "delete from `secret_friend_request` where user_unique_id = $first_id and request_unique_id = $second_id";
    $delete_request_result = mysqli_query($con, $delete_friend_request);
    if($delete_request_result) {
