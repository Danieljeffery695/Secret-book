...
<html lang="en">
  <head>
    ...
    <script defer src="user_home_post.js"></script>
    <!-- link ends here -->
  ...
  <body>
    ...
    if(isset($_POST["user_comment_show"])) {
      $user_comment = $_POST["user_comment_show"];
      $home_post_comment_id = $_POST["home_post_comment_id"];
      // $post_comment = $_POST["home_story_comment"];
      $home_post = $_POST["home_comment_post"];

      $select_comment_1 = "select * from `secret_users_home_post` where post_unique_id = $user_comment and user_id = $home_post_comment_id and user_post_img = '$home_post'";
      ...
      if($select_row_count >= 1) {
        ...
        $post_id = htmlspecialchars($select_row_data["id"]);
      } else {
        echo "nothing returned";
      }

      $select_comment = "select * from `secret_user_home_comment` where post_id = $post_id order by user_id desc";
